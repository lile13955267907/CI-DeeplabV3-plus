...
mean = SegDataFolder.mean_train
std = SegDataFolder.std_train
...
class Solver(object):
    def __init__(self, configs):
        ...
        with self.writer:
            if not self.resume:
                inp = torch.randn([1, self.channels, 128, 128]).cuda() if self.cuda else torch.randn(
                    [1, self.channels, 128, 128])
                # self.writer.add_graph(self.net,inp)
        # 在tensorboard中创建Graphs，Graphs中存放网络结构
        # activate tensorflow/tensorboard --logdir=
    ...
    def train(self, epoch):
        ...
        for i, batch in enumerate(self.train_dataloader):
            ...
            intersection, union, target = intersectionAndUnion(pred.cpu().numpy(), label.cpu().numpy(), self.n_classes,
                                                               self.ignore_index)
            ...
            if (i + 1) % self.logIterval == 0:
                logInfo = ("Train [{}/{}]\t"
                           ...
                           "Accuracy {acc:.4f}\t"
                           "IoU {iou:.4f}.").format(i + 1, len(self.train_dataloader), loss_meter=loss_meter, acc=acc,
                                                    iou=iou)
                print(logInfo)
        ...
        #  logInfo生成日志信息表，“:.4f”为保留小数点后四位
        logInfo = 'Train at epoch [{}/{}] : mIoU/mAcc/allAcc/loss {:.4f}/{:.4f}/{:.4f}/{:.4f}.'.format(epoch + 1, self.end_epoch,
                                                                                           mIoU, mAcc, allAcc,loss3)
        mIoU1 = ('{:.4f}'.format(mIoU))
        mAcc1 = ('{:.4f}'.format(mAcc))
        allAcc1 = ('{:.4f}'.format(allAcc))
        loss1=('{:.4f}'.format(loss3))

        file_hand = open('train.txt', mode="a+")
        file_hand.write(mIoU1 + ', ' + mAcc1 + ', ' + allAcc1 +', '+ loss1 + ', ' + '\n')
        file_hand.close()
        print(logInfo)
    ...
    def val(self, epoch):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(self.val_dataloader):
                ...
                intersection, union, target = intersectionAndUnion(pred.cpu().numpy(), label.cpu().numpy(),
                ...
                if (i + 1) % self.logIterval == 0:
                    logInfo = ("Val [{}/{}]\t"
                               "Loss {loss_meter.val:.4f}({loss_meter.avg:.4f})\t"
                               "Accuracy {acc:.4f}\t"
                               "IoU {iou:.4f}.").format(i + 1, len(self.val_dataloader), loss_meter=loss_meter, acc=acc,
                                                        iou=iou)
            ...
            f1 = f1_score(correct, preds, average='micro')                 # F1分数可以看作是模型精确率和召回率的一种调和平均

            if mIoU > self.best_miou:
                self.best_miou = mIoU

                self.save_state(epoch,
                                '{}/{}-{:.4f}-ep{}.pth'.format(self.result_dir, self.configs['net'], mIoU, epoch))
                self.save_pred()

            # logInfo = 'Val at epoch [{}/{}] : mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.\n'.format(epoch+1, self.end_epoch, mIoU, mAcc, allAcc)
            logInfo = 'Val at epoch [{}/{}] : mIoU/mAcc/allAcc/loss/F1 {:.4f}/{:.4f}/{:.4f}/{:.4f}/{:.4f}.'.format(epoch + 1, self.end_epoch,
                                                                                             mIoU, mAcc, allAcc, loss3, f1)
            ...
            loss2=('{:.4f}'.format(loss3))
            f12=('{:.4f}'.format(f1))
            file_hand = open('test.txt', mode="a+")
            file_hand.write(mIoU2 + ', ' + mAcc2 + ', ' + allAcc2 + ', '+ loss2 + ', ' + f12 + ',' +'\n')
            file_hand.close()
            print(logInfo)
    ...
    def test(self, prefix):
        ...
        with torch.no_grad():
            for n, img in self.trainSet.TestSetLoader():
                ...
                vutils.save_image(outp, os.path.join('./testoutput_RMAUNET', '{}_{}'.format(prefix, n)))
    ...
    def save_pred(self):
        ...
        with torch.no_grad():
            for i, batch in enumerate(self.val_dataloader):
                ...
                # 保存预测图片，像素值为0&1
                cv2.imwrite('testoutput_RMAUNET/test_output_1/{}.png'.format(path), saved_1,
                            [int(cv2.IMWRITE_JPEG_QUALITY), 95])
                # 保存预测图片，像素值为0&255
                cv2.imwrite('testoutput_RMAUNET/test_output_255/{}.png'.format(path), saved_255,
                            [int(cv2.IMWRITE_JPEG_QUALITY), 95])
...
if __name__ == "__main__":
    ...
    config = get_yaml('ConfigFiles/config-deeplab.yaml')  # 调用配置文件,更换网络的地方
    print(config)

    solver = Solver(config)
    solver.trainer()
    # solver.test(prefix='end')     tensorboard --logdir=
