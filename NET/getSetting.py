...
def get_net(config):
    if config['net'].lower() == 'UNet'.lower():
        ...
        return net
    elif config['net'].lower() == 'swin_transformer'.lower():
        net = SwinTransformer(128, 4, in_chans=config['channels'], num_classes=config['n_classes'], bilinear=True)
        return net
...
def get_criterion(config):
    if config['criterion'].lower() == 'crossentropy':
        ...
        weight = torch.FloatTensor([0.2, 0.7, 0.1])  # 修改损失函数中0与1的比重，以使曲线更平滑，0.8表示1的占比，即更关注“1”类。
        weight = weight.cuda()
...
def get_optim(config, net):
    ...
    if config['net'].lower()=='attnUnet':
        return optim.SGD(train_params,momentum=0.99,weight_decay=1e-5)

    # Define Optimizer
    if config['optimizer'].lower()=='adam':
        return optim.Adam(net.parameters(),lr=config['lr'],betas=(0.9,0.999),eps=1e-8,weight_decay=1e-5,amsgrad=True)
